...
?>
 ...
 <link rel="stylesheet" type="text/css" href="/tests/training/style.css?v=<?=filemtime($_SERVER['DOCUMENT_ROOT'] . '/tests/training/style.css')?>">
<link rel="stylesheet" type="text/css" href="/main.css?v=<?=filemtime($_SERVER['DOCUMENT_ROOT'] . '/main.css')?>">
	<meta name="viewport" content="width=device-width, user-scalable=no">
	<title>Тренировка - Тесты | Памятные Жемчужины</title>
</head>
<body>
...
.btn_sub{box-shadow: -10px -10px 15px rgba(242, 242, 242,0.5), 10px 10px 15px rgba(70,70,70,0.12);}
...
<div class="test_pblock"><p class="test_p">Тренировка Памятных Жемчужин улучшает запоминание стихов. Выберите год и квартал. После нажатия кнопки "Запустить" начнётся тренировка. Вам будет показана ссылка на одну Памятную Жемчужину. Вспомните её. После нажмите на ссылку стиха, проверьте себя и нажмите "Я ответил правильно" или "Я ответил неправильно" в зависимости от правильности ответа. Таких вопросов будет 12.</p></div>

<p>Выберите год:</p>
    <select name="year" class="yearsel" onchange="changeyear(this)">
        <option value="20">2020</option>
    </select>
    <p>Выберите квартал:</p>
...
<script>
    function changeyear(data){
        ...
        document.querySelector('.btn_sub-div a').setAttribute('href-data','/tests/training/'+selval + '/' + linkval);
    }
    function changequarter(data){
        var selval = data.value;
        var linkval = document.querySelector('.btn_sub-div a').getAttribute('href-data');
        linkval = linkval.substring(0, linkval.length - 2);
        linkval = linkval.substring(16);
        document.querySelector('.btn_sub-div a').setAttribute('href-data','/tests/training/'+linkval + '/' + selval);
    }
    
    var vall1 = document.querySelector('.quartersel').value;
    var vall2 = document.querySelector('.yearsel').value;
        var linkval = document.querySelector('.btn_sub-div a').setAttribute('href-data','/tests/training/' + vall2 + '/' + vall1);
</script>
  <div class="buttons">
